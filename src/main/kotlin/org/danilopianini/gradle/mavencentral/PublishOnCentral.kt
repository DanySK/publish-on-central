...
class PublishOnCentral : Plugin<Project> {
    ...
    private val Project.jsSourcesJar: Jar?
        get() = tasks.findByName("jsSourcesJar")?.let { it as Jar }

    private val Project.kotlinSourcesJar: Jar?
        get() = tasks.findByName("kotlinSourcesJar")?.let { it as Jar }

    private val Project.sourcesJarTasks: Set<Jar>
        get() = sequenceOf(jsSourcesJar, kotlinSourcesJar).filterNotNull().toSet()
    ...
    private fun addSourcesArtifactIfNeeded(project: Project, publication: MavenPublication, sourcesJarTask: Task) {
        if (sourcesJarTask is SourceJar) {
            if (project.sourcesJarTasks.isEmpty()) {
                publication.artifact(sourcesJarTask)
                project.logger.info(
                    "add sources jar artifact to publication ${publication.name} from task ${sourcesJarTask.name}",
                )
            } else {
                ...
                project.sourcesJarTasks.forEach {
                    ...
                    project.logger.lifecycle("remove '-js' appendix from sources jar generated by task ${it.name}")
                }
            }
        }
    }
